...
/**
 * TODO: within the scope of the test task. A real Gmail integration is stubbed out.
 */
final class GmailGateway implements Gateway
{
    private const ID_PREFIX = 'GMAIL-';
    ...
    private const IDEM_CACHE_PREFIX = 'gw:gmail:idem:';
    ...
    public function send(NotificationDto $dto): GatewayResult
    {
        ...
        if ($roll <= self::TRANSIENT_THRESHOLD) {
            return GatewayResult::transient('rate limit exceeded');
        }

        return GatewayResult::permanent('incorrect email address');
    }
}
